...
std::vector<std::pair<double, std::string>> sorted_expressions(std::vector<std::string> expressions,
    ...
    for (const auto& expr : expressions) {
        ...
        std::vector<double> scores = evaluate_fx_RPN2(expr, x_values);
...
PYBIND11_MODULE(geneticSymbolicRegressionRN_CUDA, m) {
    ...
    m.def("rpnToInfix", &rpnToInfix, "function that rpnToInfix");

    m.def("evaluate_fx_RPN2", &evaluate_fx_RPN2, "evaluate_fx improved");
}